# Truncated Quantile Critics (TQC) for Humanoid-v4 — CleanRL style
import os
import random
import time
...
class Args:
    ...
    n_critics: int = 5
    """number of quantile critic networks"""
    n_quantiles: int = 25
    """number of quantiles per critic"""
    top_quantiles_to_drop_per_net: int = 2
    """number of top quantiles to drop per critic network"""
...
class QuantileCritic(nn.Module):
    def __init__(self, obs_dim, action_dim, n_quantiles):
        super().__init__()
...
def quantile_huber_loss(quantiles, samples, device, kappa=1.0):
    # quantiles: (batch, n_total_quantiles)
    # samples: (batch, n_target_quantiles)
    pairwise_delta = samples.unsqueeze(1) - quantiles.unsqueeze(2)  # (batch, n_total, n_target)
    abs_pairwise_delta = pairwise_delta.abs()
    huber = torch.where(
    ...
    tau_hat = torch.arange(n_quantiles, device=device, dtype=torch.float32) / n_quantiles
    # tau_hat shape: (n_total_quantiles,) -> broadcast over batch and target dims
    loss = (torch.abs(tau_hat.unsqueeze(0).unsqueeze(2) - (pairwise_delta < 0).float()) * huber).mean()
    return loss
...
if __name__ == "__main__":
    ...
    critics = [QuantileCritic(obs_dim, action_dim, args.n_quantiles).to(device) for _ in range(args.n_critics)]
    target_critics = [QuantileCritic(obs_dim, action_dim, args.n_quantiles).to(device) for _ in range(args.n_critics)]
    ...
    # number of quantiles to keep after truncation
    n_target_quantiles = args.n_critics * args.n_quantiles - args.top_quantiles_to_drop_per_net * args.n_critics
        ...
        for global_step in range(1, args.total_timesteps + 1):
            ...
            if global_step >= args.learning_starts:
                ...
                with torch.no_grad():
                    ...
                    # get target quantiles from all target critics
                    all_target_quantiles = []
                    for tc in target_critics:
                        all_target_quantiles.append(tc(data["next_obs"], next_actions))
                    # (batch, n_critics * n_quantiles)
                    all_target_quantiles = torch.cat(all_target_quantiles, dim=1)
                    # sort and truncate (drop top quantiles)
                    sorted_quantiles, _ = torch.sort(all_target_quantiles, dim=1)
                    truncated_quantiles = sorted_quantiles[:, :n_target_quantiles]
                    # Bellman target
                    target_quantiles = data["rewards"].unsqueeze(-1) + (1 - data["dones"].unsqueeze(-1)) * args.gamma * (truncated_quantiles - alpha * next_log_pi)

                # compute quantile huber loss for each critic
                total_critic_loss = torch.tensor(0.0, device=device)
                for c in critics:
                    current_quantiles = c(data["obs"], data["actions"])  # (batch, n_quantiles)
                    loss = quantile_huber_loss(current_quantiles, target_quantiles, device)
                ...
                # use mean of all critic quantiles for actor loss
                all_q = []
                for c in critics:
                    all_q.append(c(data["obs"], pi_actions).mean(dim=1, keepdim=True))
                mean_q = torch.cat(all_q, dim=1).mean(dim=1, keepdim=True)
                actor_loss = (alpha * log_pi - mean_q).mean()
                ...
                if global_step % 1000 == 0:
                    ...
                    writer.add_scalar("losses/value_loss", total_critic_loss.item() / args.n_critics, global_step)
